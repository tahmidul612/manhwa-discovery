...
- Analyzes commit history to determine version bumps
- Updates version numbers in tracked files
- Generates and updates the CHANGELOG
- Creates git tags following semantic versioning

### Version Bumping Rules

Based on conventional commits, version increments follow:

| Increment | Trigger | Example |
|-----------|---------|---------|
| **MAJOR** (x.0.0) | Breaking changes | `feat!:`, `BREAKING CHANGE:` |
| **MINOR** (0.x.0) | New features | `feat:` |
| **PATCH** (0.0.x) | Bug fixes, improvements | `fix:`, `perf:`, `refactor:` |
...
```bash
# Automatically bump based on commits since last tag
uv run cz bump
...
uv run cz changelog --dry-run "$(uv run cz version -p)"
```

### Pre-commit Hooks

The project includes pre-commit hooks for automatic commit message validation:
...
Once installed, hooks will:
- ✅ Validate commit messages follow conventional format
- ✅ Check branch name conventions before push
...
Version tracking is configured in `pyproject.toml`:
...
- ⚠️ **Major version zero**: While the project is in initial development (`0.x.x`), breaking changes bump MINOR instead of MAJOR
- 📝 **Changelog format**: Follows [Keep a Changelog](https://keepachangelog.com/) format
- 🏷️ **Tag format**: Tags use the format `v{version}` (e.g., `v0.2.0`)
- 🔒 **Annotated tags**: All version tags are annotated with release information
